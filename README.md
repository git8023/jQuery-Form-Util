...
# Main API
  ...
  getFormItems(refreshCached) | `refreshCached`类型:Boolean, true-使用缓存的表单项, false-刷新表单项缓存(表单项有变动时使用当前值) | 表单项控件列表对象[formItem]( "也可使用$form.formItem直接访问"). 其中`formItem.singleValItems`保存只接受一个值的表单控件(也包含select[multiple]); `formItem.repeatableItems`保存接受多个值的表单项控件
